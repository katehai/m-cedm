...
from models.losses import UnnormalizedLoss
from models.normalizer import set_input_unnorm
...
class PlFno2d(pl.LightningModule):
    def __init__(self, hparams):
        super().__init__()
        self.save_hyperparameters()
        self.model = Fno2d(hparams)
        ...
        self.mae_un_inp_criterion = UnnormalizedLoss(nn.L1Loss())

    def setup(self, stage: str = None) -> None:
        if stage == "fit":
            # set the data normalization statistics
            datamodule = self.trainer.datamodule
            self.set_norm_stats(datamodule.get_norm_stats())
        return

    def set_norm_stats(self, stats):
        set_input_unnorm(self, stats.norm_input, stats.input_mean, stats.input_std)

    def forward(self, u: torch.Tensor, dx: torch.Tensor, dy: torch.Tensor, dt: torch.Tensor):
        return self.model(u, dx, dy, dt)
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        u_history = u[:, :self.model.time_history]  # B, T_history, H, W
        u_future = u[:, self.model.time_history:]  # B, T_future, H, W
        T_future = u_future.shape[1]

        u_hat = []
        inp = u_history
        for t in range(T_future // self.model.time_future):
            y_hat = self.forward(inp, dx, dy, dt)
            u_hat.append(y_hat)
            if self.teacher_forcing:
                inp = u_future[:, t * self.model.time_future:(t + 1) * self.model.time_future]
            else:
                inp = y_hat
        ...
        mae_un_loss = self.mae_un_inp_criterion(u_hat, u_future)

        self.log('train_loss', loss, prog_bar=True)
        self.log('train_mae_loss', mae_loss, prog_bar=True)
        self.log('train_mae_un_loss', mae_un_loss, prog_bar=True)
    ...
    def validation_step(self, val_batch, batch_idx):
        ...
        self.log('val_mae_un_loss', mae_un_loss, prog_bar=True)
